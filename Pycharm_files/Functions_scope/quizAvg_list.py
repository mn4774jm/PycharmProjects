...
def processing(numStudents, numQuizzes): #variables from loop arguments
    ...
    for student in range(numStudents):
        ...
        #Calculate student average; moved outside of quiz loop
        studentAverage = studentTotal / numQuizzes
        #call loopOutputs to print individual data; moved outside quiz loop
        loopOutputs(studentTotal, studentAverage, student, studentScorelist)
        #Add student total points to overall total at top of processing; outside of quiz loop
        overallTotal += studentTotal
        #Add each students list to list of lists; outside of quiz loop
        listOflists.append(studentScorelist)
    #Overall average calculation assigned; outside student loop
    overallAverage = overallTotal / numStudents / numQuizzes
    return listOflists, overallAverage #outside student loop

#Def function for individual quiz scores
def loopOutputs(studentTotal, studentAverage, student, studentScorelist):
    #(+1) for correct student count.
